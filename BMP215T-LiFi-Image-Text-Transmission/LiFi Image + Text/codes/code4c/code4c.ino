...
void serial_get_command()
{
  ...
  if(Serial.available() > 0)
  {
    ...
    { 
      ...
      if(inchar == '}')
      {
        Serial.print("Count : ");Serial.println(arr_count);
        for(i=0;i<arr_count;i++)
        {
          /*Serial.print(i);Serial.print(":");*/Serial.print(arr[i]);
        }
        lcd.clear();lcd.print("Received : ");lcd.print(arr_count);
        rec_flag1=1;
    ...
    if(inchar == '<')
    {
      ...
      if(inchar == '>')
      {
        ...
        Serial.print("Text Received : ");Serial.println(cmd_arr2);
        lcd.clear();lcd.print("Rec. Text: ");lcd.print(cmd_count-1);
      }
      else
      {
        Serial.println("<EE04>");    
...
void loop()
{
  ...
  if(rec_flag1==1)
  {
    ...
    if (buttonState == LOW)
    {
      lcd.setCursor(0, 1);lcd.print("Sending Image...");
      delay(1000);
      GSerial.print("<");
      for(i=0;i<arr_count;i++)
      {
  ...
  if(rec_flag2==1)
  {
    ...
    if (buttonState == LOW)
    {
      lcd.setCursor(0, 1);lcd.print("Sending Text... ");
      delay(1000);
      GSerial.print("{");
